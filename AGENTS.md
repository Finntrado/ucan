# AGENTS.md — U-CAN Website Redesign

Working guide for continuing the **urban.org.in** (Urban Collective Action Network / U-CAN) website redesign in Codex. Read this fully before touching any page.
...
**Repo layout (Codex, persistent) — mirrors the dastangoi project's `standalone/` convention so Vercel deploys cleanly:**
```
ucan/
├── AGENTS.md
...
**User working style:** casual, direct, action-oriented. Wants complete copy-ready files (never snippets), verbatim source content, strong emphasis on zero dead space. When asked to redesign pages, **use Codex Sonnet 4.6** (the user's stated model preference for this project).
...
**This rule is saved in the user's Codex memory.** Treat it as always-on.
...
Do this directly by hand-editing the HTML in Codex. The earlier Python-fragment build scaffolding (`<page>_data.py` / `build_<page>.py` / `assemble_<page>.py`) was a sandbox convenience from the original chat-based session — it's not needed here and shouldn't be reintroduced.
...
9. Model choice is up to the user's current Codex default; the "Sonnet 4.6" note was specific to the original chat-based session and doesn't necessarily carry over.
