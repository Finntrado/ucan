...
- **The header's inner nav is `<nav class="ucnav-bar">`, never `class="nav"`.** `index.html`'s own stylesheet still defines `.nav` — that class *was* its header before unification — so a shared header using it inherited a `border-bottom` (the stray line across the homepage header) and 40px of extra width, which shifted the whole bar on three pages. Same trap exists for `.header` (newsletter) and `.masthead` (the brief): **never give a chrome element a class that any page stylesheet already styles.**
- The chrome stylesheet **owns the cookie banner** too. `.cc` used to be defined only in the shell's base64 stylesheet, so on `newsletter.html` and the June 2026 brief the banner rendered as an undismissable block at the end of the page instead of a fixed bottom popup.
...
## 23. Chrome bleed — the bug class to watch for

Three symptoms, one root cause, worth internalising before touching `chrome.py`:

1. **The header shifted slightly between pages.** `.ucnav` measured 526.5px wide on
   index / newsletter / the June 2026 brief but 486.5px on every shell page.
2. **A stray hairline across the homepage header.**
3. **The cookie banner appeared as an undismissable block** at the end of
   `newsletter.html` and the brief, instead of a bottom popup.

All three were **page stylesheets bleeding into shared chrome**. The header's inner
container was `<nav class="nav">`, and `index.html` still styles `.nav` because that
class used to be its header element — so it inherited that page's `border-bottom` and
padding. The banner was the mirror image: chrome markup relying on CSS that only *one*
of the three base64 stylesheets defines.

**The rule:** every shared chrome element gets a class no page stylesheet can already
match (`ucnav-bar`, `bar-in`, `f-sub`, …), and the injected chrome stylesheet defines
**every** property that element's appearance depends on — including a small reset
(`margin/padding/border/background/line-height`) so inherited rules cannot leak in.

`test_js.js` now measures `.bar`, `.bar-in`, `.brand`, `.ucnav` and `.bar-cta` on one
...
## 30. Canonical host, HTTPS, robots.txt, llms.txt (`wp-theme/ucan/inc/canonical.php`)

**Rule (user, standing): the canonical origin is ALWAYS `https://www.urban.org.in`, HTTPS is always enforced,
every crawler incl. AI ones may read every public page, and security is never weakened. Look for SEO/AEO/GEO
opportunities on every page-level change (see the `seo-on-every-change` memory).**

Found by probing the live site before changing anything: all four variants (http/https × apex/www) returned
200 with no redirects (four URLs per page); canonicals said the apex; `www/robots.txt` was 301'd to the apex by
WordPress core; `http://…/wp-login.php` served the login form in clear text; `/?author=1` redirected to
`/author/admin/` (username disclosure). `www` already served the same site with a valid certificate.

- **Public pages**: http or apex → one 301 (308 for non-GET) to `https://www.urban.org.in<path>`.
- **wp-admin / wp-login / REST / cron / xmlrpc**: HTTPS forced, **host never changed** (login cookies and the
  WPVibe connector are tied to the configured host, and a redirected POST loses its body).
- **Canonical/hreflang/og:url/og:image/twitter:image/JSON-LD** are rewritten to the www origin at serve time
  (`ucan_canonicalise_head`) whatever host served the request; sitemap + robots `Sitemap:` line too. In-page
  links and assets stay on the request's own origin (`ucan_public_origin` / `ucan_on_origin`), so LocalWP and
  previews still work. **No change to WordPress `home`/`siteurl`** (the connector blocks that option anyway).
- **robots.txt** (filter, priority 99): `Allow: /` for `*` and 16 named search/AI/answer-engine bots, keeping
  `Disallow` for `/wp-admin/` (admin-ajax allowed), `/wp-login.php`, `/wp-json/`, `?s=`. "Discourage search
  engines" is respected. It is a courtesy, not a control: nothing sensitive is listed.
- **`/llms.txt`**: generated by `_scripts/seo/build_llms_txt.py` from the real pages, served by an explicit route
  as `text/plain`. Urban Perspectives is omitted until those pages are live — then re-run with
  `--with-perspectives`. No engine has confirmed llms.txt affects ranking; it is cheap and harmless.
- **`redirect_canonical` is disabled on production hosts** (it bounced www → apex) and `allowed_redirect_hosts`
  includes www + apex — without that, `wp_safe_redirect` silently sends trailing-slash/old-URL redirects to wp-admin.
- **Tests**: `python _scripts/seo/test_canonical.py` — 34 scenarios in php-cgi (real headers): every host/scheme
  combo, POST 308, REST/admin never host-redirected, CRLF header injection, head rewriting, robots, llms.txt.
